...
> - [Part 1](https://tony-waters.github.io/2026/03/03/demo-spring-rest-app.html): build a modern Spring REST application using Aggregates, Command Query Separation, and sensible unit tests
...
> - [Part 3](https://tony-waters.github.io/2026/05/28/Terraform-an-application.html): use Terraform to create the local development environment and add Prometheus and Grafana observation

---
---

I wrote a [simple non-trivial Spring REST application](https://github.com/tony-waters/spring-boot-app) to experiment with the following technologies:
...
The basic idea of `aggregates` is that by fencing entities into logical groups (each group called an `aggregate`) and controlling access to this group through a single entity within the group (called the `aggregate root`) it is easier to prevent invalid state because the invariants are encapsulated. Think Facade pattern, where one of the original Classes gets promoted (for good reason) to the Facade. If there is no obvious candidate to take charge of the group, maybe the group is more than one group. [Identifying aggregates and roots is not necessarily a simple process](https://softwareengineering.stackexchange.com/questions/355647/find-the-ddd-aggregate-root).
...
If thats clean, you can try running using `TEST_PROFILE=load` and `TEST_PROFILE=stress`. Be aware that 'stress' may cause some failures (it does on my laptop). Which can be very useful.
