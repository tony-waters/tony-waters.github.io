...
Circuit-breaker and bulkhead are complementary resilience patterns, but address different failure modes.  Circuit Breakers prevent repeated calls to failing dependencies by opening the circuit when error rates exceed a threshold, enabling fail-fast behavior and giving the external service time to recover.  Bulkheads isolate system resources (such as thread pools or connection pools) into independent compartments, ensuring that a slowdown or failure in one dependency does not exhaust resources and starve other critical services. 
